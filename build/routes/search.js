...
async function searchRoutes(app) {
    app.get("/search", async (req) => {
        ...
        const { images, videos } = await (0, media_1.getUploads)();
        ...
        const usersFiltered = q.length === 0
            ? users
        ...
        const postsFiltered = q.length === 0 ? posts : posts.filter((p) => p.caption.toLowerCase().includes(q) || p.username.toLowerCase().includes(q));
        ...
        const reelsFiltered = q.length === 0
            ? reels
